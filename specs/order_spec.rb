...
describe "Order Wave 1" do
  ...
  describe "#remove_product" do
    it "can delete the product from the product collection" do
      products = { "banana" => 1.99, "cracker" => 3.00 }
...
#
  ...
  describe "Order.find" do
    ...
    it "Returns nil for an order that doesn't exist" do
      nil_order = Order.find(19594)

      expect(nil_order).must_be_nil
    end
  end

  describe "Order.find_by_customer" do
    it "Can find the orders using a customer id" do
      orders_list = Order.find_by_customer(25)

      expect(orders_list).must_be_kind_of Array
      expect(orders_list.length).must_equal 6
    end

    it "Returns nil for an order that doesn't exist" do
      orders_list = Order.find_by_customer(275825)

      expect(orders_list).must_be_nil
