...
#
  ...
  describe "Order.find_by_customer" do
    ...
    it "Returns accurate information about the first order" do
      orders = Order.find_by_customer(14)
      expect(orders[0].id).must_equal 6
      expect(orders[0].customer).must_be_kind_of Customer
      expect(orders[0].customer.id).must_equal 14
      expect(orders[0].fulfillment_status).must_equal :pending
    end
